...
module Review {
  ...
  function onGettingToken(responseStatus, token) {
    logger.info("Response received for review token request:: " + responseStatus);
    if (responseStatus == WatchUi.REVIEW_REQUEST_STATUS_GRANTED and itsOk()) {
      if (token != null) {
        logger.info("The request has been approved. Launching a user review request.");
  ...
  function itsOk() {
    logger.debug("Starting to check the ability to request a review from a user");
    ...
    if (dateFirstStart == null) {
      ...
      logger.debug("Skipping the review request. Less than 4 days have passed since I started using the app");
      return;
    }
    ...
    if (!(booksKeys instanceof Lang.Array) or booksKeys.size() == 0) {
      logger.debug("Skipping the review request. There are no books in the playlist");
      return false;
    }
    ...
    if (lastReviewRequestTime != null) {
      if (
        ...
        logger.debug("Skipping the review request. Review requested less than 20 days ago");
        return false;
      }
    }
    ...
    if (
      ...
      logger.debug(
        "Skipping the review request. A review version "+currentVersion+" has already been requested" 
      );
      return false;
    }
