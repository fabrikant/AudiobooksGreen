...
1. Для работы приложения ТРЕБУЕТСЯ промежуточный прокси, который пропускает через себя запросы от часов и ответы от сервера. И вот почему:
...
В любом случае Audiobooks Green по умолчанию сначала делает прямой запрос к вашему серверу audiobookshels и только при получении ошибки -402 (Serialized response was too large), повторяет запрос через прокси.
...
Но чтобы быть совершенно спокойным, вам придется проделать то же самое с **[прокси серврером](https://github.com/fabrikant/AudiobooksGreenProxy)** и развернуть его на вашем оборудовании.
...
Исходный код и инструкции по установке собственного прокси сервера вы можете получить перейдя по **[ссылке](https://github.com/fabrikant/AudiobooksGreenProxy)**