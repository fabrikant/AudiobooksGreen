...
1. Реализованы только методы GET, POST, DELETE. Такие методы как PUT, PATCH или UPDATE отсутсвуют напрочь
...
2. Для работы приложения ТРЕБУЕТСЯ промежуточный прокси, который пропускает через себя запросы от часов и ответы от сервера. И вот почему:
    ...
    - Но главная боль заключается не в этом. Главная проблема в том, что API архитектура audiobookshelf сделана неоптимально (это мое мнение, которым я не хочу кого-то обидеть). В чем собственно проблема. API audiobokshelf возвращает огромные (нет, не так ОГРОМНЫЕ) объемы НЕНУЖНЫХ в данном контексте данных. Кромет того, в API практически полностью отсутсвует pagination. 
...
Для удобства пользователей приложения, я встроил путь к прокси в приложение и совершенно бесплатно предоставляю доступ к своему прокси серверу. Я сам являюсь пользователем [Audiobooks Green](https://apps.garmin.com/en-EN/apps/0fbb79e9-1b2b-41e6-96a2-32679b484db4) и вынужден держать прокси сервер для себя. Нагрузка на него не большая и я вполне могу поделиться его ресурсами с сообществом.

В ответ на это, я получил со стороны комьюнити audiobookshelf некоторый шейминг, так как схема работы приложения не прозрачна. С чем я полностью согласен. И что возникает угроза безопасности учетным данным сервера вашего сервера и, что, если я являюсь злоумышленником, я получу доступ к вашим серверам. И это правда. 

Особенный упор почему-то делался на наличие прокси сервера, как основного источника угрозы. Почему, я так и не понял. Мне ничто не мешает угнать ваши учетные данные прямо из приложения [Audiobooks Green](https://apps.garmin.com/en-EN/apps/0fbb79e9-1b2b-41e6-96a2-32679b484db4)?

Короче говоря, да. Если вы пользуетесь программой из [стора](https://apps.garmin.com/en-EN/apps/0fbb79e9-1b2b-41e6-96a2-32679b484db4) и/или моим прокси, вы находитесь под угрозой, что я сопру ваши учетные данные и буду нахаляву слушать вашего Терри Пратчета.

Не хотите приниать такой ужасный риск, но хотите слушать книги с часов? У меня для вас
...
Чтобы быть совершенно спокойным, вам придется проделать то же самое с прокси серврером и развернуть его на вашем собственном оборудовании.
